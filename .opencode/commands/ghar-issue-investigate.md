...
- Anything else → Fail this workflow
...
**If not a Github issue:**

- Stop and ask the user for a Github issue
...
Use Task tool with subagent "Explore" (if available):
...
**Path:** `.ghar/issues/issue-{number}.md`

If free-form (no issue number): `.ghar/issues/investigation-{timestamp}.md`
...
Post the complete artifact content to GitHub:

````bash
gh issue comment {number} --body "$(cat .ghar/issues/issue-{number}.md)"
````

**PHASE_5_CHECKPOINT:**
- [ ] Complete artifact content posted to GitHub (if GH issue)
...
{Posted to issue | Skipped (no Github issue)}
...
- **GITHUB_POSTED**: Complete artifact posted to issue (if GH issue)
